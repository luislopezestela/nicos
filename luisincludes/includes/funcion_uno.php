...
function lui_GetMedia_bl($media) {
    global $wo;
    if (empty($media)) {
        return '';
    }
...
function lui_UploadLogo_mobil($data = array()) {
    global $wo, $sqlConnect;
    if (isset($data['file']) && !empty($data['file'])) {
        $data['file'] = $data['file'];
    }
    ...
    $dir      = "datos/modulos/" . $wo['config']['theme'] . "/img/";
    $filename = $dir . "icono.{$file_extension}";
    if (move_uploaded_file($data['file'], $filename)) {
        $check_file = getimagesize($filename);
        if (!$check_file) {
            unlink($filename);
        ...
        if (lui_SaveConfig('logo_mobil_extension', $file_extension.'?cache='.rand(100,999))) {
            return true;
        }
    }
}
...
function lui_ShareFile_desk($data = array(), $type = 0, $crop = true) {
    global $wo, $sqlConnect, $s3;
    $allowed = '';
    if (!file_exists('upload/files/' . date('Y'))) {
        @mkdir('upload/files/' . date('Y'), 0777, true);
