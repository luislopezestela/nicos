...
foreach ($users as $key => $value) {
	$wo["user"] = lui_UserData($value['user_id']);
	if ($wo["user"]["pro_type"] == 0) {
		$update      = lui_UpdateUserData($wo["user"]["id"], array(
            "is_pro" => 0,
            'verified' => 0,
            'pro_' => 1
        ));
...
if ($wo['config']['live_video'] == 1) {
	...
	if (!empty($user)) {
		$wo['user'] = lui_UserData($user['user_id']);
		$wo['loggedin'] = true;
		if ($wo['config']['live_video_save'] == 0) {
	        try {
	            ...
	            foreach ($posts as $key => $post) {
	                ...
	                lui_DeletePost(lui_Secure($post->id),'shared');
	            }
	        } catch (Exception $e) {
