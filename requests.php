...
$s = '';
if (isset($_GET['f'])) {
    $f = lui_Secure($_GET['f'], 0);
}
if (isset($_GET['s'])) {
    $s = lui_Secure($_GET['s'], 0);
}
$hash_id = '';
...
//if (!in_array($f, $non_login_array)) {
  //  if ($wo['loggedin'] == false && ($s != 'load_more_posts')) {
    //    if ($s != 'load-comments') {
      ///      exit("Por favor iniciar sesion o registrarse para continuar hello world.");
        //}
    //}
//}
...
elseif (!empty($_GET['mode_type']) && in_array($_GET['mode_type'], array('linkedin','instagram'))) {
    include 'xhr/modes/' . lui_Secure($_GET['mode_type']) . '.php';
}
