<tr class="setting-userlist <?php echo ($wo['report']['seen'] == 0) ? ' report-not-seen' : '';?> " id="ReportID_<?php echo $wo['report']['id']?>" data_selected="<?php echo($wo['report']['id']) ?>">
   ...
   <td>
      ...
      <button class="btn bg-danger admn_table_btn" onclick="Wo_DeleteReportedContent(<?php echo $wo['report']['user']['id']?>,<?php echo $wo['report']['id']?>,'user','hide');">Eliminar usuario</button>
      <button class="btn bg-info admn_table_btn" onclick="$('#read_report').modal().find('.report_text').text('<?php echo htmlspecialchars($wo['report']['text']); ?>')">Ver informe</button>
      <?php if ($wo['report']['user']['id'] != $wo['user']['user_id']) { ?>
      <?php if ($wo['report']['user']['banned'] == 1) { ?>
         <button class="btn bg-secondary admn_table_btn" onclick="Wo_UnBanUser(<?php echo $wo['report']['user']['id'] ?>,'hide');">Desbanear usuario</button>
      <?php }else{ ?>
      <button class="btn bg-secondary admn_table_btn" onclick="Wo_BanUser(<?php echo $wo['report']['user']['id'] ?>,'hide');">Banear usuario</button>
      <?php } ?>
      <?php } ?>
   </td>
</tr>