    ...
    <div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Categorias de blog</li>
            </ol>
        </nav>
    </div>
    <!-- Vertical Layout -->
                  ...
                  <h6 class="card-title">Categorias</h6>
                   ...
                   <div class="table-responsive1">
                        <table class="table table-bordered table-striped table-hover">
                            ...
                            <tbody>
                                <?php 
                                $categories_keys = lui_GetCategoriesKeys(T_BLOGS_CATEGORY);
                                foreach ($wo['blog_categories'] as $category_id => $category_name) {
                                  $wo['category_key'] = $category_id;
                                  $wo['category_name'] = $category_name;
                                  $wo['category_lang_key'] = $categories_keys[$category_id];
                                	echo lui_LoadAdminPage('blogs-categories/list');
                                }
                                ?>
                            </tbody>
                        </table>
...
<div class="modal fade" id="SelectedDeleteModal" tabindex="-1" role="dialog" aria-labelledby="exampleModal1Label" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                Estas seguro que quieres eliminar las categorias seleccionados?
            </div>
            <div class="modal-footer">
...
add_category_form.ajaxForm({
    url: Wo_Ajax_Requests_File() + '?f=admin_setting&s=add_new_category&type=blog',
    beforeSend: function() {
...
edit_category_form.ajaxForm({
    ...
    beforeSend: function() {
        edit_category_form.find('.waves-effect').text("Espere por favor..");
    },
    success: function(data) {
        if (data.status == 200) {
            edit_category_form.find('.waves-effect').text('Save');
            $('.edit_category_form_alert').html('<div class="alert alert-success"><i class="fa fa-check"></i> Categoria editado con exito</div>');
            setTimeout(function () {
...
function DeleteSelected() {
    ...
    $.post(Wo_Ajax_Requests_File()+"?f=admin_setting&s=remove_multi_category&type=blog", {ids: data}, function () {
...
function Wo_DeleteCat(id,type = 'show') {
  ...
  $.post(Wo_Ajax_Requests_File() + '?f=admin_setting&s=delete_category&type=blog', {lang_key: id}, function(data, textStatus, xhr) {});
}
