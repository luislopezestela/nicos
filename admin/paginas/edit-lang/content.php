...
$langs    = $db->objectbuilder()->paginate(T_LANGS, $page,array('lang_key',lui_Secure($_GET['id'])));
$langs = ToArray($langs);
if ($db->totalPages != 0) {
    if (($page > $db->totalPages) && !empty($_GET['page-id'])) {
        header("Location: " . lui_LoadAdminLinkSettings('manage-languages'));
        exit();
    }
}
...
<div class="container-fluid">
    <div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Administrar Lenguages</li>
            </ol>
        </nav>
    </div>
    <!-- Vertical Layout -->
    <div class="row">
        <div class="col-lg-12 col-md-12">
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title">Editar Lenguage ISO</h6>
        ...
        <div class="col-lg-12 col-md-12">
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title">Administrar & Editar Lenguages</h6>
                    <div class="row">
                       <div class="col-md-6" style="margin-bottom:0;">
                            <form method="get" action="<?php echo lui_LoadAdminLinkSettings("edit-lang"); ?>">
                              <div class="row">
                                <div class="col-md-8">
                                    <div class="form-group form-float">
                                        <div class="form-line">
                                            <label class="form-label search-form">Buscar por palabra</label>
                   ...
                   <div class="table-responsive1">
                            ...
                            <tbody>
					            ...
					            if (count($langs) > 0) {
					                ...
					                foreach ($langs as $key => $wo['langs']) {
					                	...
					                	$wo['lang_id'] = $count++;
					                    echo lui_LoadAdminPage('edit-lang/list');
					                }
					            }  
					            ?>
                            </tbody>
                    ...
                    <div class="pull-right">
                        <nav>
                            <ul class="pagination">
                                <li>
                                    <a href="<?php echo lui_LoadAdminLinkSettings("edit-lang?id=".$lang_name."&page-id=1").$link; ?>" data-ajax="?path=edit-lang&id=<?php echo($lang_name); ?>&page-id=1<?php echo($link); ?>" class="waves-effect" title='First Page'>
                                        <i class="material-icons">first_page</i>
                                    </a>
                                </li>
                                <?php if ($page > 1) {  ?>
                                <li>
                                    <a href="<?php echo lui_LoadAdminLinkSettings("edit-lang?id=".$lang_name."&page-id=" . ($page - 1)).$link; ?>" data-ajax="?path=edit-lang&id=<?php echo($lang_name); ?>&page-id=<?php echo($page - 1) ?><?php echo($link); ?>" class="waves-effect" title='Previous Page'>
                                ...
                                for ($i=$nums_pages; $i <= $db->totalPages; $i++) { 
                                    if ($nums < 20) {
                                    ?>
                                    <li class="<?php echo ($page == $i) ? 'active' : ''; ?>"><a href="<?php echo lui_LoadAdminLinkSettings("edit-lang?id=".$lang_name."&page-id=" . ($i)).$link; ?>" data-ajax="?path=edit-lang&id=<?php echo($lang_name); ?>&page-id=<?php echo($i) ?><?php echo($link); ?>" class="waves-effect"><?php echo $i ?></a></li>
                                    <?php
                                    }
                                    $nums++;
                                }
                                ...
                                <li>
                                    <a href="<?php echo lui_LoadAdminLinkSettings("edit-lang?id=".$lang_name."&page-id=" . ($page + 1)).$link; ?>" data-ajax="?path=edit-lang&id=<?php echo($lang_name); ?>&page-id=<?php echo($page + 1) ?><?php echo($link); ?>" class="waves-effect" title="Next Page">
                                        <i class="material-icons">chevron_right</i>
                                    </a>
                                </li>
                                <?php } ?>
                                <li>
                                    <a href="<?php echo lui_LoadAdminLinkSettings("edit-lang?id=".$lang_name."&page-id=" . ($db->totalPages)).$link; ?>" data-ajax="?path=edit-lang&id=<?php echo($lang_name); ?>&page-id=<?php echo($db->totalPages) ?><?php echo($link); ?>" class="waves-effect" title='Last Page'>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content ">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-success" onclick="Wo_SubmitLangForm()" data-dismiss="modal">Guardar cambios</button>
            </div>
        </div>
    </div>
</div>