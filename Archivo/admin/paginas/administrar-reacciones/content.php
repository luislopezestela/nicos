<?php $wo['reactions_types'] = lui_GetReactionsTypes(''); ?>
<div class="container-fluid">
    <div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Reacciones</li>
            </ol>
        </nav>
    </div>
    <!-- Vertical Layout -->
    <div class="row">
        <div class="col-lg-12 col-md-12">
            <div class="card">
                <div class="card-body">
                  <h6 class="card-title">Agregar nueva reacción</h6>
                  <div class="alert alert-info">Puede agregar nuevas reacciones desde aqui, tenga en cuenta que debe escribir el nombre de la reaccion en cada idioma.</div>
                  <div class="row">
                                ...
                                <div class="col-md-2" id="normal-query-form">
                                  <div class="form-group form-float">
                                      <div class="form-line focused">
                                        <label class="form-label">Reaccion de Layshane</label>
										<div class="btn-file d-flex align-items-center">
											<input type="file" id="wowonder-icon" accept="image/x-png, image/gif, image/jpeg" name="layshane" class="hidden">
											...
											<div class="full-width">
												<b id="wowonder-icon-name">Icono</b>
											</div>
										</div>
                                      </div>
                                  </div>
                        ...
                        <table class="table table-bordered table-striped table-hover">
                            ...
                            <tbody>
                                ...
                                foreach ($wo['reactions_types'] as $reaction) {
                                  ...
                                  $wo['reaction_layshane_star'] = $reaction['layshane_star'];
                                  $wo['reaction_status'] = $reaction['status'];
                                	echo lui_LoadAdminPage('administrar-reacciones/list');
                                }
                                ?>
                            </tbody>
                        </table>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModal1Label">Eliminar reaccion</h5>
...
<div class="modal fade" id="editcategoryModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content modal_content_back">
      <div class="modal-header">
        <h5 class="modal-title" id="editcategoryModalLabel">Editar reaccion</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-success" id="save_edited_reaction">Guardar cambios</button>
      </div>
    </div>
  </div>
</div>
...
<style type="text/css">
	...
	#wowonder-icon-name, #sunshine-icon-name {font-weight: normal;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;display: block;}
</style>

<script>
	$("#wowonder-icon").change(function () {
		var filename = $(this).val().replace(/C:\\fakepath\\/i, '');
		$("#wowonder-icon-name").text(filename);
    });
  function ReactionStatus(self,id) {
    $(self).text('Por favor espere..');
  ...
  function EditReaction(id) {console.log(id)
...
add_reaction_form.ajaxForm({
    ...
    beforeSend: function() {
        add_reaction_form.find('.btn-info').text("Por favor espere..");
    },
    success: function(data) {
      add_reaction_form.find('.btn-info').text('Agregar');
        if (data.status == 200) {
...
edit_reaction_form.ajaxForm({
    ...
    beforeSend: function() {
        $('#editcategoryModal').find('.btn-success').text("Por favor espere..");
    },
    success: function(data) {
        if (data.status == 200) {
            $('#editcategoryModal').find('.btn-success').text('Guardar cambios');
