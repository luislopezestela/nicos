						...
						<div class="card-body">
							<div class="d-flex align-items=center theme <?php echo ($theme == $wo['config']['theme']) ? ' active' : '';?>">
								...
								<div>
									...
									<?php } else { ?>
										<div class="active">
                                            <a id="active" data="<?=$codigo;?>" value="<?=$theme;?>" class="btn btn-primary">Activar tema</a>
                                        </div>
                                        <input style="padding:7px;border-radius:5px;outline:none;margin-top:7px;border:2px solid #ccc;width:100%;" type="text" name="code_activation" class="code_activation<?=$theme;?>" placeholder="Codigo de activacion ">
...
$(function() {
   $('a#active').click(function() {
    $('#theme_name').val($(this).attr('value'));
        var copd = $(this).attr('data');
        let id_t = $(this).attr('value');
        var numd = $('.code_activation'+id_t).val();
        if (copd == numd){
