<?php
 if ($wo['config']['shout_box_system'] == 1 && lui_CheckAnonymous($wo['reply']['id'],'reply')) {
  $anonymous = true;
   $wo['reply']['publisher']['username'] = 'anonymous';
   $wo['reply']['publisher']['name'] = $wo['lang']['anonymous'];
   $wo['reply']['publisher']['avatar'] = lui_GetMedia('upload/photos/incognito.png');
...
<div class="reply reply-container" id="comment_reply_<?php echo $wo['reply']['id'];?>" data-reply-id="<?php echo $wo['reply']['id'];?>">
  <?php if (!isset($anonymous)) { ?>
   <a class="<?php echo lui_RightToLeft('pull-left'); ?>" href="<?php echo $wo['reply']['publisher']['url']?>" onclick="InjectAPI('{&quot;profile_id&quot; : &quot;<?php echo $wo['reply']['publisher']['id']?>&quot;, &quot;type&quot;:&quot;<?php echo $wo['reply']['publisher']['type']?>&quot;}', event);">
   <img class="avatar <?php echo lui_RightToLeft('pull-left'); ?>" src="<?php echo $wo['reply']['publisher']['avatar']?>" alt="avatar">
   </a>
   <?php }else{ ?>
    <img class="avatar <?php echo lui_RightToLeft('pull-left'); ?>" src="<?php echo $wo['reply']['publisher']['avatar']?>" alt="avatar">
  <?php } ?>
      ...
      <div class="comment-heading reply-heading">
        ...
        <?php } ?>
         ...
         <div class="<?php echo lui_RightToLeft('pull-right');?> comment-options comment_edele_options">
            <?php if ($wo['reply']['post_onwer'] === true|| $wo['reply']['onwer'] === true || lui_IsAdmin()) {?>
         ...
         <div class="comment-image">
            <?php if (!empty($wo['reply']['c_file'])): ?>
               <span data-href="<?php echo lui_GetMedia($wo['reply']['c_file']);?>" onclick="Wo_OpenLighteBox(this,event);" class="pointer">
               <img src="<?php echo lui_GetMedia($wo['reply']['c_file']);?>" alt="Comment image">
               </span>
            <?php endif ?>
         </div>
        ...
        <?php if ($wo['config']['second_post_button'] == 'reaction') { ?>
            ...
            <div class="like-stat stat-item" id="comment_reacted">
              ...
              <?php echo lui_GetPostReactions($wo['reply']['id'],"replay");?>
              </span>
            </div>
        <?php } ?>
   ...
   <?php echo lui_LoadPage('modals/delete-comment-reply');?>
</div>