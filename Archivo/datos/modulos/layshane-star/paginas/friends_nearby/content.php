<div class="page-margin">
	...
	<?php if (lui_IsAdmin() && strpos($wo['config']['site_url'], 'https') === false): ?>
	...
	<div class="page-margin">
		<div class="row wow_frnd_usr" id="nearby_users_list">
			<?php
				$nearby_users = lui_GetNearbyUsers(array('country' => $wo['user']['country_id']));
				if (count($nearby_users) > 0) {
					foreach ($nearby_users as $wo['UsersList']) {
						echo lui_LoadPage('friends_nearby/includes/user-list');
					}
				}
				else{
