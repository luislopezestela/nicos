	...
	<div class="wo_story_header">
		<div class="wo_stor_h_info">
			...
			<div class="wo_s_h_i_r">
				...
				<span class="ajax-time" title="<?php echo date('c',$wo['story']['posted']); ?>"><?php echo lui_Time_Elapsed_String($wo['story']['posted']); ?></span>
			</div>
		</div>
		<div class="wo_stor_h_opts">
			<?php if (!$wo['story']['is_owner']) { ?>
			<div class="close-lightbox like-story-lightbox <?php echo(lui_IsReacted($wo['story']['id'],$wo['user']['id'],'story') ? 'active' : '') ?>" data-story-id="<?php echo $wo['story']['id']; ?>">
			...
			<div class="like-stat stat-item post-like-status" style="<?php if ($wo['story']['is_owner']) {?> float:left; <?php }else{ ?> float:right; <?php }?>">
		      ...
		      <?php echo lui_GetPostReactions($wo['story']['id'],'story');?>
		      </span>
		    </div>
		    <?php } ?>
		    <?php if (!$wo['story']['is_owner']) { ?>
			<ul class="story_reacts_box reactions-box reactions-box-story-container-<?php echo $wo['story']['id']; ?>" data-id="<?php echo $wo['story']['id']; ?>">
					<?php if (!empty($wo['reactions_types'])) {
					    foreach ($wo['reactions_types'] as $key => $value) {
					     ...
					     ?>
						<li style="<?php if ($wo['story']['is_owner']) {?> left: 10px; <?php }else{ ?> right: 10px; <?php }?>" class="reaction reaction-<?php echo $value['id'];?>" data-reaction="<?php echo $value['name'];?>" data-reaction-id="<?php echo $value['id'];?>" data-reaction-lang="<?php echo $value['name'];?>" data-post-id="<?php echo $wo['story']['id']; ?>" onclick="Wo_RegisterStoryReaction(this,'<?php echo($value['layshane_star_small_icon']) ?>',<?php echo($value['is_html']) ?>);">
