...
//lui_GetCategories_sections
function lui_GetCategories_sections($table,$section) {
    global $sqlConnect, $wo;
    $data       = array();
    $categories = mysqli_query($sqlConnect, "SELECT * FROM " . $table . " WHERE `id_section` = '{$section}'");
        ...
        if ($table == "lui_products_categories") {
            ...
            if(!empty($categories)){
                while ($fetched_data = mysqli_fetch_assoc($categories)) {
                    $fetched_data["id_section"] = $fetched_data["id_section"];
                    $fetched_data["logo"] = lui_GetMedia($fetched_data["logo"]);
                    $data[$fetched_data["id"]] = $fetched_data;
                }
            }
...
function lui_GetSectionCategories($table) {
    global $sqlConnect, $wo;
    $data       = array();
    $sections = mysqli_query($sqlConnect, "SELECT * FROM " . $table);

    if (mysqli_num_rows($sections)) {
        while ($fetched_data = mysqli_fetch_assoc($sections)) {
            $data[$fetched_data["id"]] = $wo["lang"][$fetched_data["lang_key"]];
        }
        return $data;
    }
...
function lui_GetSectionCatKeys($table) {
    global $sqlConnect, $wo;
    $data       = array();
    $sections = mysqli_query($sqlConnect, "SELECT * FROM " . $table);
    if (mysqli_num_rows($sections)) {
