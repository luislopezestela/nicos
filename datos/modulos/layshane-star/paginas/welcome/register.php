...
.container_acceder{width:100%;height:calc(100vh - 160px);display:grid;grid-template-columns:1fr;grid-gap:7rem;padding:0 2rem;}
...
.terms label::after,.terms label::before{display:inline-block;left:0;}
.terms label{padding-left:30px;cursor: pointer;user-select: none;font-size: 16px;opacity: 0.9;position:relative;}
...
.btn{display:block;width:100%;height:50px;border-radius:25px;outline:none;border:none;background-image:linear-gradient(to right, #53b4ea, #01deff, #ffffff);background-size:200%;font-size:1.2rem;color:#fff;font-family:'Poppins', sans-serif;text-transform:uppercase;margin:1rem 0;cursor:pointer;transition:777ms;}
.btn:hover{background-position:right;display:block;background-image:linear-gradient(to left, #00a4ff, #00deff, #36afca);transform:scale(104%);}
