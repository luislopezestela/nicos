...
.buscador_layshane_s1 .bar {
  position: relative;
  display: block;
  width: 200px;
}
.buscador_layshane_s1 .bar:before,.buscador_layshane_s1 .bar:after {
  content: '';
  height: 2px;
  width: 0;
  bottom: 1px;
...
.buscador_layshane_s1 .bar:before{left:50%;}
.buscador_layshane_s1 .bar:after{right:50%;}
.buscador_layshane_s1 .input:focus ~ .bar:before,
.buscador_layshane_s1 .input:focus ~ .bar:after{width: 50%;}
</style>
<style type="text/css">
.con_layshane_tbles{
	margin-top:30px;
		...
		<div class="profile-lists singlecol">
			<div class="avatar-holder mt-0">
				...
				<?php if (!isset($comprapendiente->completado)): ?>
					...
					<?php } ?>
				<?php else: ?>
					<button class="btn_prin_compra boton_add_nluis first cancelar_order_in_pages"><?php echo $wo['lang']['anular_compra'] ?></button>
				<?php endif ?>
			</div>
			<br><br>
			    	...
			    	<div class="contenido_datos_new_compra" id="<?='compra_pendiente'.$comprapendiente->id;?>">
			    		...
			    		<div class="comprobante">
				    		...
				    		<style type="text/css">
								...
								.atributes_listas_compras input[type="radio"]:checked + label .radio-circle {
								  border-color: #fff;
								  background-color: #3498db;
								}

								.atributes_listas_compras input[type="radio"]:checked + label .radio-text {
								  color: #FFF;
								}
								...
								.precio_compra_invo_s .precio_compra_inputs[type="number"]::-webkit-outer-spin-button {
								  -webkit-appearance: none;
								  margin: 0;
								}
								.input_number_laysh[type="number"],
								.precio_compra_invo_s .precio_compra_inputs[type="number"] {
								...
								.btn_layshane-1{
								 ...
								 background:linear-gradient(0deg, #f94141 0%, rgb(255, 95, 95) 100%);
								 border: none;
								}

								.btn_layshane-1:before {
								 height: 0%;
								 width: 2px;
								}

								.btn_layshane-1:hover {
			    									...
			    									<table class="table" style="background-color:#F8F8F8;">
			    										...
			    										<tbody class="table__tbody content_resuls_sseach">
				                							...
				                							foreach ($posts as $value) {
				                								...
				                								echo lui_LoadPage('compras/list_items');
				                							} ?>
				                						</tbody>
			    									</table>
			    								</div>
			    							...
			    							<div class="columna-12">
			    								...
			    								<div class="contenido_ct_footer_document_order">
			    									<div class="footer_document_order" style="gap:0.2em;">
			    										...
			    										<input class="input_number_laysh update_garant_dt" type="number" value="<?=$cantidad_de_garantia;?>" autocomplete="off"><br>
				    				...
				    				$('.endcompra').click(function(e) {
									    ...
									    if(!button.hasClass('animatebtncompra')) {
									        ...
									        $.ajax({
												...
												data: {price_dat:untformat,fecha:dat},
												type: 'POST',
								...
								$('.contet_items_de_doc_compr').on('keyup','.precio_compra_inputs_a',function(){
									...
									$.ajax({
										...
										success: function (data){
											...
											$('.precio_compra_inputs_totalsub_'+numbers_ac_pl).html(tsubs);
											$('#items_st_total').html(data.total_items);
											$('#cantidad_st_total').html(data.total_lista);
											$('#price_st_total').html(data.total_precio);
										}
			...
			<script type="text/javascript">
				function hideElements() {
				    ...
				    $('.submenu_add_irtd_docmt').slideUp(); // Oculta los elementos .submenu_add_irtd_docmt
				    $('.placeholder_atri').slideUp(); // Oculta los elementos .placeholder_atri
				    $('body').removeClass("menu_atrr"); // Elimina la clase "menu_atrr" del body
				}
			...
			<div class="col-lg-12 col-md-12">
	            <div class="card">
	                ...
	                <div class="conten_footer_tabla_layshane">
	                	...
	                	<div class="row">
	                		<div class="columna-lg-3 columna-3">
	                			<span>&nbsp;</span>
	                			<button type="button" class="button3 delete-selected" disabled>Eliminar lo seleccionado<span></span></button>
	                		</div>
	                	</div>
	                	<div class="clearfix"></div>
	                </div>
