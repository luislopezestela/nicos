					...
					<div class="col-md-6">
						<div class="wow_form_fields">
							...
							<select name="bidding" id="bidding" class="form-control">
								<option value="clicks" <?php echo ($wo['ad-data']['bidding'] == 'clicks') ? 'selected' : ''; ?>><?php echo str_replace('${{PRICE}}',  lui_GetCurrency($wo['config']['currency']) . $wo['config']['ad_c_price'], $wo['lang']['pay_per_click']); ?></option>
								<option value="views" <?php echo ($wo['ad-data']['bidding'] == 'views') ? 'selected' : ''; ?>><?php echo str_replace('${{PRICE}}', lui_GetCurrency($wo['config']['currency']) . $wo['config']['ad_v_price'], $wo['lang']['pay_per_imprssion']); ?></option>
							</select>
						</div>
					</div>
				</div>
	...
	jQuery(document).ready(function($) {
	   ...
	   <?php } ?>
      	...
      	$('#update-ads').ajaxForm({
	       	...
	       	type:"POST",
	       	beforeSend: function() {
	        	$('#update-ads').find('.add_wow_loader').addClass('btn-loading');
	       	},
	       	success: function(data) {
	        	...
	        	if (data['status'] == 200) {
	       		...
	       		$('#update-ads').find('.add_wow_loader').removeClass('btn-loading');
      		}
  		});
	});
</script>