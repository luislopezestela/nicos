<div class="messages-wrapper {{#if onwer}} pull-right {{else}} pull-left {{/if}} messages-text"
	...
	<div class="message {{#if onwer}} outgoing pull-right {{else}} incoming pull-left {{/if}}"
		onclick="Wo_ShowMessageOptions({{chatmsgId}});" {{#unless wowonderTheme}} style="background: {{backgroundColor}}; color:{{color}};" {{/unless}}>
		
		{{#if chatTxt}}
			{{#if onwer }}
			<p class="message-text" dir="auto" style="background: {{backgroundColor}}; color:{{color}};">
				{{#if hasHTML}}
					{{{chatTxt}}}
				{{else}}
					{{chatTxt}}
